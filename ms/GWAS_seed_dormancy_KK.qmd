...
phs_dist<-phs_setup%>%group_by(taxa)%>%
  ...
  labs(x="PHS score(0-9)",y="Frequecny")+ggtitle(label = paste0("Histogram of Preharvest Sprouting \n values for 2020/2021"))+
...
op<-df_begin_long%>%rbind(df2)%>%mutate(headerFacet = 'Dataset Year') %>%
  ...
  labs(title = 'Germination Energy and Index over time \n by individual genotype')+theme_bw()
op
...
#All chromosomes
WinterPerTPGWAS2
ALL_MM<-WinterPerTPGWAS2%>%ungroup()%>%filter( log10PVal>2)%>%mutate(trait=ifelse(trait=="GI0","GI",trait))%>% ggplot(aes(ordinal, log10PVal, color = as.factor(PM)))+geom_point()+ggtitle(label="Genome wide association for Germination Energy and Index \n across timepoints and years")+guides(color=guide_legend(title="Timepoint"))+
...
ALL_MM
CH5_MM<-WinterPerTPGWAS2%>%ungroup()%>%filter( log10PVal>2)%>%mutate(trait=ifelse(trait=="GI0","GI",trait))%>% ggplot(aes(ordinal, log10PVal, color = as.factor(PM)))+geom_point(size = 1.5)+ggtitle(label="Genome wide association for \n Germination Energy, Index and PHS on CH 5")+guides(color=guide_legend(title="Timepoint"))+
