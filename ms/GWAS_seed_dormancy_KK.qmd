...
df_begin_long<-df_begin%>%mutate(GI_0=GI/GE)%>%mutate(GI_0=ifelse(is.nan(GI_0),NA,GI_0))%>%pivot_longer(names_to="trait",cols=c("GE","GI","GI_0"))%>%rename(year=Year)
df2<-df_begin_long%>%filter(PM_date%in%c("5","19","47","96","152"))

df_begin%>%group_by(Year,PM_date)%>%dplyr::summarize(cor(GE,GI_0,use="pairwise.complete.obs"))
df2$year<-"2020/2021"
library(arm)
...
phs_setup<-df_begin_long%>%rbind(df2) %>% mutate(year = factor(year, levels = c('2020','2021','2020/2021')))%>%
  plyr::join(gd_Q[,c('taxa','Qsd1')]) %>% filter(Qsd1!= 1) %>% mutate(AlaAT= ifelse(Qsd1==2,'NonDormant','Dormant')) %>% plyr::join(gd_Q[,c('taxa','JHI_Hv50k_2016_8507')]) %>% filter(JHI_Hv50k_2016_8507!= 1) %>% mutate(JHI_Hv50k_2016_8507= ifelse(JHI_Hv50k_2016_8507==2,'minor','ref'))%>%
  filter(Family %nin% c('Lightning'))  %>%select(phs,Qsd1,AlaAT,taxa,SourcePLOT,Location,year)%>%unique()
...
# Results
Correlations between PHS, GI and GE at most timepoints. PHS scores were averaged across years as there was a lack of sufficent phs data in the 2020 year alone. The high correlation between traits and timepoints suggests strong genetic control for phs and seed dormancy loss. Higher correlations were observed at later timepoints in 2021 compared to 2020.
Low to moderate narrow sense heritability was observed for pre-harvest sprouting in the 2020 and 2021 years(h2 = 0.37) (Table 1) Distribution of phs scores skewed highly toward the left as most phs scoresgenotype were classified as resistant, classified as green (Figure 1). Endeavor and Charles, had consistently high phs susceptible scores. Narrow sense heritaiblity for GE was high at early timepoints and dropped as genotype reached peak dormancy loss. Narrow sense heritability for GI increased during the beginning of the after ripening period and decreased although at a slower rate than GE, as x percent of lines did not reach 90% germination at PM67 for 2020 and PM96 for 2021. 
Even after dormancy loss, most experimental lines GI scores peaked at 6, consistently lower than the average GI observed for spring malting barley populations was dormancy loss was complete @sweeney. Seed dormancy was consistently higher in all genotypes in the 2021 year compared to the 2020 year, with the exception of check lines of Charles in 2020 and Endeavor in 2021. 
...
```
## Distrbutions
A geom smooth plot using a GLM loess method was used to the timepoint data for GE and GI within and across years. There was significantly more dormancy in all experimental lines and parent checks in the 2021 harvest year. A total of 5 timepoints were measured for the 2020 year and increased to 8 for the 2021 year. Mean dormancy loss occured at approximatley 50-70 days post PM in 2020 and approximateley 67-80 days post PM in 2021.
