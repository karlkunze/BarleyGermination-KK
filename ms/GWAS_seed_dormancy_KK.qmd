...
tt<-STP_all%>%ungroup()%>%drop_na(pos)
View(tt)
ch2<-tt[tt$chrom=="2",]
View(ch2)
table(STP_all$ID)
...
table_chr<-tt%>%ungroup()%>%dplyr::select(marker,chrom,pos,ordinal)%>%arrange(chrom,pos)%>%drop_na(pos)%>%mutate(ordinal=plyr::mapvalues(marker,from=ord_list$marker,to=ord_list$ordinal),ordinal=as.numeric(ordinal))%>%unique()
ord_list$marker
marker
table_chr
