...
#replace with
grep("JHI-Hv50k-2016-31660", colnames(myGD20))

#TP2 5H marker
grep("JHI-Hv50k-2016-311497",colnames(myGD20))
ld_heatmap(myGD20[,c(5149:5150,5289,5441)])
###
colnames(myGD20)[5149]<-"AlaAT_L214F"
ld_heatmap(myGD20[,c(5149:5150,5289,5441)]) + ggtitle('LD between AlaAT_L214F marker \n and most significant hit at 5H') +
  theme(axis.text.x = element_text(angle = 90))+
  

  scale_x_discrete(label = c("AlaAT_L214F","-308652","-311497","-316599"), breaks = c("AlaAT_L214F","JHI-Hv50k-2016-308652","JHI-Hv50k-2016-311497","JHI-Hv50k-2016-316599"))+
  scale_y_discrete(label = c("AlaAT_L214F","JHI-Hv50k-2016-308652","JHI-Hv50k-2016-311497","JHI-Hv50k-2016-316599"), breaks = c("AlaAT_L214F","JHI-Hv50k-2016-308652","JHI-Hv50k-2016-311497","JHI-Hv50k-2016-316599"))+theme(axis.text.x = element_text(angle = 90,size = 10,face = "bold"),axis.text.y = element_text(face = "bold"))+
  xlab(NULL)+ylab(NULL)
...
#family haplotype function

Graph_haplotype_family=function(marker,trait,TP_value,list_value){
...
df_haplotype_m=df_haplotype_m[!is.na(df_haplotype_m$Family),]
plot=ggplot(data=df_haplotype_m, aes(x=Family, y=value,color=df_haplotype_m[,marker])) +
  ...
  ylab(paste0(list_value))+
  scale_fill_manual(values=c("#cccccc", "#636363")) +
guides(color=guide_legend(paste(marker)))+
  theme(plot.title = element_text(hjust = 0.5,size=10,face = "bold"),axis.text.x = element_text(angle =0,size = 9,face = "bold"),legend.position = "none")
...
#plot.PHS
marker<-'AlaAT_L214F'
table(myGD20$AlaAT_L214F)
1-169/305
TP2GEqsd1=Graph_haplotype_family('AlaAT_L214F','GE','TP2', 'GE BLUE')
TP2GIqsd1=Graph_haplotype_family('AlaAT_L214F','GIscale','TP2','GI BLUE')
#TP2GE.legend=Graph_haplotype_family_legend('AlaAT_L214F','GIscale','TP2','GI BLUE')
TP2GE.legend
library(gridExtra)
#TP1GE_5Dqsd1=Graph_haplotype_family('AlaAT_L214F','GE_5D','TP1')
figure<-grid.arrange(TP2GEqsd1,TP2GIqsd1)
figure
#AALLL
...
df_haplotype=df_haplotype[!is.na(df_haplotype[,marker]),]
df_haplotype=df_haplotype[!df_haplotype[,marker]=="1",]
df_haplotype$type=NA

df_haplotype[,marker]=as.factor(df_haplotype[,marker])
df_haplotype_m=df_haplotype[df_haplotype$TP==TP_value,]
df_haplotype_m[df_haplotype_m[,marker]=="0",]$type="Dormant"
df_haplotype_m[df_haplotype_m[,marker]=="2",]$type="Non-dormant"
table(df_haplotype_m$type,df_haplotype_m$Family)
df_haplotype_m=melt(df_haplotype_m, id.vars =c('taxa','Family',marker), measure.vars = c(trait))
df_haplotype_m$AlaAT_L214F
df_haplotype_m=df_haplotype_m[!is.na(df_haplotype_m$Family),]
plot=ggplot(data=df_haplotype_m, aes(x=Family, y=value,color=AlaAT_L214F)) +
  geom_boxplot()+
  #facet_grid(cols=vars(dataset), scales="free", rows=vars(variable)) +
  theme_bw() + 
  ...
  scale_color_manual(labels = c('Non-dormant','Dormant'),values = c("#F8766D","#00BFC4")) 

#guides(color=guide_legend(paste(marker)))+
  #theme(plot.title = element_text(hjust = 0.5,size=10,face = "bold"),axis.text.x = element_text(angle =0,size = 9,face = "bold"))

####
plot
legend.T=get_legend(plot)
legend<-grid.arrange(legend.T)
vplot
plot(legend)
return(lenend.T)
