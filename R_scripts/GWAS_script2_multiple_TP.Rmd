...
```{r}
getwd()
source("R_scripts/GWAS_script.Rmd")

library(ASRgwas)
#library(ASRgenomics)
gwasMTP <- ASRgwas::gwas.asreml(pheno.data = pre.aprA$pheno.data,resp = "GI",randomf = c("Year:PM_date"),
 gen = "GID", Kinv = pre.aprA$Kinv,
Q =NULL, npc = 5, geno.data = pre.aprA$geno.data,
map.data = pre.aprA$map.data, pvalue.thr = 5e-04,
bonferroni = FALSE, fixedf =c("Year"))
...
ls(gwasFIN)
```
#make a function for asreml GWAS
```{r}
...
wmbGAPIT$GD[1:5,1:5]
wmb_GD_01=as.data.frame(apply(wmbGAPIT$GD,1, function(x) { 
  x[which(x == 0)] = -1
  x[which(x == 1)] <- 0
  x[which(x == 2)] = 1
...
wmb_GD_01[1:5,1:5]
#wmb_GD_01$taxa=rownames(wmb_GD_01)



FoldCVGPv2 = function(df, myGD=WinterGD, numfolds,datasplit, trait_name){
#df =AllDHBluesPerYear%>%filter(year=="2020")%>% filter(TP=='TP1', trait == 'GE',type =='BLUE');myGD = t(wmb_GD_01); numfolds = 10;datasplit = .5;trait_name ='GE'
...
library(rrBLUP)
wmb_GD_01
DH2020Estimates=AllDHBluesPerYear%>%filter(year=="2020")
GETP22020= FoldCVGPv2(DH2020Estimates%>% filter(TP=='TP1', trait == 'GE',type =='BLUE'),myGD = t(wmb_GD_01), numfolds = 10,datasplit = .5,trait_name ='GE' )
GETP22020

AllDHBluesPerYear
GETP22020= FoldCVGPv2(df = DH2020Estimates %>% filter(TP=='TP2', trait == 'GE',type =='BLUE'),myGD = t(wmb_GD_01), numfolds = 10,datasplit = .5,trait_name ='GE' )
...
Taxacounts2020 = DH2020Estimates %>% filter(type == 'BLUE', trait == 'GE') %>% ungroup() %>% group_by(taxa) %>% 
  ...
  filter(taxa %in% rownames(t(wmb_GD_01)))
...
Taxacounts2021 = DH2021Estimates %>% filter(type == 'BLUE', trait == 'GE') %>% ungroup() %>% group_by(taxa) %>% 
  summarise(count = n()) %>%
  filter(taxa %in% rownames(t(wmb_GD_01)))
DH2020Estimates=AllDHBluesPerYear
...
library(Hmisc)
 ...
 WinterGD =  wmb_GD_01%>%t()%>%as.data.frame() %>%tibble::rownames_to_column(var="taxa")%>%relocate(taxa,.before = 1)
  WinterGD 
...
GE_TP1_2020_model_Train = mixed.solve(y = GE_TP1_2020 %>% arrange(taxa)%>%filter(taxa%in%WinterGD$taxa) %>% ungroup()%>%arrange(taxa) %>%
                                        select(value) %>% as.matrix(),
                                       Z = WinterGD %>% filter(taxa%in%GE_TP1_2020$taxa)%>% arrange(taxa) %>%
                                        select(!taxa) %>% as.matrix(),
                                      K=NULL,
                                      SE=FALSE)
