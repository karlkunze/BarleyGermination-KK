...
marker_ef<-genoD%>%as.data.frame()%>%dplyr::select(c("Qsd1","JHI_Hv50k_2016_308762"))%>%tibble::rownames_to_column()%>%dplyr::rename(GID=rowname)%>%mutate(Qsd1=as.factor(Qsd1),JHI_Hv50k_2016_308762=as.factor(JHI_Hv50k_2016_308762))
marker_ef
...
Q =pre.aprM$Q, npc = 2, geno.data = pre.aprM$geno.data,
...
gd[1:5,1:5]
simple<-simple%>%dplyr::select(SourcePLOT,GID,Year,PM_date,replication,Location,Check,Day1Germ,scald,Qsd1,GE,GI,GI_0)
simple$PM_date_n<-as.numeric(as.character(simple$PM_date))
library(asreml)

v=asreml(fixed = GI ~ 1 + grp(Q) + Year:PM_date + Year, 
    random = ~+vm(GID, pre.aprM$Kinv$GID), residual = ~dsum(~units | 
        Year), data = pre.aprM$pheno.data, na.action = list(x = "include", 
        y = "include"), group = list(Q = c(5, 6)), workspace = "1Gb")
summary(v)
predict.asreml(v,classify = "GID")

?predict.asreml()
single_step<-asreml(fixed=y~E+at(E,5):Planting_dateM,random=~G+E:G,residual = ~dsum(~ar1(Column):ar1(Row) + id(Column):ar1(Row) +ar1(Column):id(Row) | E, levels = list(c(1,6), c(7,8,5),c(2,3,4))))
...
df_sum=df_begin%>%rename(year=Year)%>%group_by(year,TP,GID)%>%summarise(GIm=mean(GI,na.rm=TRUE),GEm=mean(GE,na.rm=TRUE),GI0m=mean(GI_0,na.rm=TRUE))
table(df_sum$year)
#tidyr::pivot_longer(cols=c("GI","GE","GI_0"),names_to = "trait")
...
df= AllDHBluesPerYear%>%filter(year=="2020")%>% filter(TP=='TP1', trait == 'GE')
...
DH2021_GE.logfits = G2021.TSF.GE %>% ungroup()  %>%
   rbind(G2021.TSF.GE %>% mutate(PM_date = PM_date,year = '2021'),
       DHCombined.TSF.GE  %>% mutate(PM_date = PM_date,year = '2020/2021'))%>%
  arrange(taxa, TP) %>% group_by(year, taxa) %>% group_modify(GE_logFits_trycatch) %>% ungroup() 
...
TaxaToFilter2021GE = DH2021_GE.logfits %>% filter(year == '2021')%>% 
  filter(term=='Centering' & estimate> 200 | term=='TimeTo95' & estimate>250)
TaxaToFilter2021GE
DH2021_GE.logfits %>% ggplot(aes(x = estimate)) +geom_density()+facet_wrap(vars(term), scales = 'free')
TaxaToFilter2021GE 
###end 2021
#Combined
DH2021_GE.logfits
...
```{r}
DH2021_GE.logfits$year<-"2021"
DHCombined_GE.logfits
dim(DH2021_GE.logfits)
DH2021_GE.logfits=DH2021_GE.logfits%>%filter(year=="2021")
...
GWA_MLMM_fortidyR = function(df, groupvars) {
   ...
   GWAS = GAPIT(Y = df %>% dplyr::select(taxa, value) %>% as.data.frame(),GD=WinterGD%>%as.data.frame(), GM=wmb_GM%>%as.data.frame(),PCA.total = 2,
...
table(GE.logfits$term,GE.logfits$year)
GE.logfits$year
library(dplyr)
table(GE.logfits$term)
 
simple21<-GE.logfits%>%as.data.frame()%>% dplyr::filter(year%in%c('2021'))%>%filter(term=="Centering")
simple21<-simple21%>% dplyr::rename(value = estimate,GID=taxa)%>%filter(!GID%in%TaxaToFilter2021GE$taxa)%>%dplyr::left_join(QSD1,by="GID")

simple21
genoMp=genoM;maf_lim=0.05;p.val=5e-05;bonfer=FALSE
names(simple21)
rownames(gd)
pre.aprM <- pre.gwas(pheno.data=simple21, indiv = "GID",
...
random=NULL
resid=NULL
hist(pre.aprM$pheno.data$value)


#(.packages())
gwasMTP_log <- gwas.asreml(pheno.data = pre.aprM$pheno.data,resp = "value",
 gen = "GID", Kinv = pre.aprM$Kinv,
Q =pre.aprM$Q, npc = 2, geno.data = pre.aprM$geno.data,fixedf = fix,randomf = random,residual = resid,
map.data = pre.aprM$map.data,bonferroni = FALSE,pvalue.thr = 1e-05, message = TRUE)


qq.plot(gwas.table = gwasMTP_log$gwas.all)

manhattan.plot(gwas.table = gwasMTP_log$gwas.all, pvalue.thr = 5e-05,
point.size = 1.2)
...
DH2020_GE.logestimates$color<-"black"
DH2020_GE.logestimates[DH2020_GE.logestimates$time<20&DH2020_GE.logestimates$GE_est<0.50,]$color<-"A"
DH2020_GE.logestimates[DH2020_GE.logestimates$time>30&DH2020_GE.logestimates$GE_est>0.5,]$color<-"A"
DH2020_GE.logestimates
DH2020_GE.logestimates %>% filter(taxa %nin% TaxaToFilter2020GE$taxa) %>% ggplot(aes(x = time, y = GE_est,color=color, group = taxa)) +geom_line()
...
DH2020_GIlogfits
taxaToFilter2020GI = DH2020_GIlogfits  %>% dplyr::filter(year=='2020') %>% 
...
```{r}
library(dplyr)
DH2020_GIlogfit = DH2020_GIlogfits  %>%
  filter(!(year == '2020' & taxa %in% taxaToFilter2020GI$taxa)) %>%
  # Add filtering steps here
  group_by(year,term) %>%  dplyr::rename(value = estimate) 

DH2020_GIlogfit
```
```{r}
DH2020_GIlogfits$estimate
DHGI2020.logestimates = DH2020_GIlogfits  %>% filter(term %in% c('Centering','Rate','Upper','Lower')) %>% group_by(year, taxa)
DH2020_GI.logfits

DHGI2020.logestimates%>%mutate(estimate=ifelse(estimate=="NaN",0,estimate))%>%
...
TaxaToFilterGI=list(taxaToFilter2020GI,taxaToFilter2021GI,taxaToFilter20202021_GI)
save(TaxaToFilterGE,file="data/Phenotype_Data/log_fits/GE_log_taxa_to_filter.Rdata")
...
DHGI20202021.logestimates = DH20202021_GIlogfits %>% filter(term %in% c('Centering','Rate','Upper','Lower')) %>% group_by(year, taxa) %>% 
  group_modify(~{data.frame(time = t,GI_est= .x$estimate[2]+(.x$estimate[3]-.x$estimate[2])/(1+exp(.x$estimate[1]*(log(t)-log(.x$estimate[4])))))})
...
```{R}
getwd()
source('~/git/TimeSeriesGermination/SpringBarley/Analysis/All_taxa/FPCA/Functions/FPCA_function.R')
source('~/git/TimeSeriesGermination/SpringBarley/Analysis/All_taxa/FPCA/Functions/pca_fun.R')
source('~/git/TimeSeriesGermination/SpringBarley/Analysis/All_taxa/FPCA/Functions/pca_score.R')
source('~/git/TimeSeriesGermination/SpringBarley/Analysis/All_taxa/FPCA/Functions/tuning_nointer.R')
library(drc)
library(fda)
library(magic)
...
GEGI2020FPCATimeTo = data.frame(time = GI2020FPCA$EstimatedAndEmpiricalMu$time[-c(1:5)]) %>% cbind(GI2020FPCA$RecoveredCurves)  %>% pivot_longer(cols = !time, names_to='taxa') %>%
  ...
  dplyr::select(taxa, trait, year, Param, time) %>% dplyr::rename(value=time)

#GWA using the FPCs without interpolation and penalized spline fits to the data
GWA2020.FPCA.GEGI = rbind(GI2020FPCA$PCs_withTaxa %>% dplyr::select(taxa, FPC1,FPC2) %>% mutate(trait='GI', year= '2020') %>%
                            pivot_longer(cols = c(FPC1,FPC2), values_to = 'value',names_to='Param'),
                          GE2020FPCA$PCs_withTaxa %>% dplyr::select(taxa, FPC1,FPC2) %>% mutate(trait='GE', year= '2020') %>%
