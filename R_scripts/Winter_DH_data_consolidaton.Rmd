 ...
 mutate(Check=ifelse(!grepl("BS",Entry), 1, 0),
         ...
         Family = mapvalues(substr(taxa,1,3), from = c('BS6','BS7','BS8','BS9','DH1','Fla','SY_','Sca','Win','KWS'), 
                            to = c('Flavia/DH130910','Scala/DH130910','SY_Tepee/DH130910','Wintmalt/DH130910',
                                   'DH130910','Flavia/DH130910','SY_Tepee/DH130910','Scala/DH130910','Wintmalt/DH130910','Scala/DH130910')))%>%select(-taxa0)%>%mutate(Year=as.factor(Year))
...
DH_2020_PHS<-as.data.frame(read.csv("data/Winter_DH/WMB_DH_2020_Ketola_PHS.csv",stringsAsFactors = TRUE))%>%dplyr::rename(sprout_score=Sprout.Score)%>%select(Entry,PLOT,sprout_score)%>%mutate(sprout_score=as.character(sprout_score),sprout_score=replace(sprout_score,sprout_score=="0","00000"))%>%na.omit()%>%arrange(PLOT)
...
All2020<-left_join(TP2020,DH_2020_field_traits,by=c("PLOT","taxa","Family","Year"))
All2020=All2020%>%dplyr::rename(SourcePLOT=PLOT,seed_mass=mass,HD_date=HD,AssayNumber=Sample_ID)%>%select(-sprout_score,-Entry)
...
#detach("package:plyr", unload=TRUE)
WBMaster21=WBMaster21%>%rename(test_wt="Test Wt",SourcePLOT=PLOT21,grain_name="GRAIN NAME",ws_percent=ws,PowderyMildew="PoweryMildew")%>%select(Entry,SourcePLOT,Location,Year,Check,Row,Column,kgha,ws_percent,frost_damage,HD_date,TP_date,Maturity_date,Ht,Lodging,phs,Scald,SB,PowderyMildew)%>%
 mutate(HD_date=as.numeric(format(WBMaster21$HD_date, "%j")),TP_date=as.numeric(format(WBMaster21$TP_date,"%j")))%>%mutate(taxa=gsub("KWS Scala","Scala",Entry))%>%dplyr::rename(TipPoint_date=TP_date)%>%select(-Entry)
