...
numeric_matrix<-as.numeric(as.matrix(wdh[c(1:nrow(wdh)),c(2:ncol(wdh))]))
numeric_matrix[which(numeric_matrix==1)]= 2
numeric_matrix[which(numeric_matrix==0.5)]= 1
wdh[c(1:nrow(wdh)),c(2:ncol(wdh))]<-numeric_matrix
wdh[c(1:8),c(1:8)]

rm(numeric_matrix)

wdh[which(wdh ==1)] = 2
wdh[which(wdh == 0.5)] = 1 #these should be removed, we shouldn't have heterozygous positions with double haploids
