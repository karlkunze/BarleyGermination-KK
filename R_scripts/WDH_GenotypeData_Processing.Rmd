...
Morex2019<-read.delim("/home/karl/git/BarleyGermination-KK/data/Genotype_data/barley50kMarkerPositions_Morex2019Assembly.gff",skip=1,header = FALSE)
...
Morex2019
```
Morex V3, do not have strand values
```{r}
library(readxl)
library(stringr)
MorexV3 <- read_excel("data/Genotype_data/SNPPositions_50k_on_MorexV3.xlsx")
colnames(MorexV3)<-c("Marker","Chrom","pos")

MorexV3$Chrom<-sapply(str_split(MorexV3$Chrom,"r"),`[`,2)
colnames(Morex2019)
Morex2019
MorexV3$Type<-"barley50kChip"
MorexV3$strand<-NA
MorexV3=MorexV3[,c(2,4,3,5,1)]
MorexV3$version<-"V3"
```
