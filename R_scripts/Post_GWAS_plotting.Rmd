...
vb_m
object<-round(cor(vb_m,use="pairwise.complete.obs",method="spearman"),2)
object.pval<-cor.test(vb_m)
vb_m
object<-rcorr(as.matrix(vb_m),type="pearson")
...
predict.asreml(df[[1]]$mod$coefficients)
df[[1]]$mod$coefficients
df[[1]]$mod$coefficients$random
var_test<-df[[1]]$gwas.all%>%arrange(chrom,pos)%>%mutate(ordinal=row_number(.),fdr.pvalues=p.adjust(p.value,method="fdr"))
