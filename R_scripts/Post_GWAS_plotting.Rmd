...
GE20<-convert(GE_2021)

GI_C<-convert(GI_STP_combined)
GE_C<-convert(GE_STP_combined)
STP_all<-rbind(GE20,GI20,GI_C,GE_C)%>%mutate(PM=as.numeric(str_split_i(ID,"_",2)),trait=str_split_i(ID,"-",1),year=str_split_i(ID,"-",2))%>%mutate(log10PVal=-log10(p.value))%>%arrange(year,trait,PM)
STP_all
###
...
WinterPerTPGWAS2 %>%ungroup()%>% ggplot(aes(ordinal, log10PVal, color = as.factor(PM), shape = year))+geom_point()+
 ...
 # geom_vline(xintercept = 4780, color = 'red',linetype='longdash')+
  ...
  ylab('-log(p-value)')+xlab('Chromosome')+ geom_hline(yintercept = -log10(1e-5) )+theme_bw()+
