...
library(asreml)
getwd()
setwd("/home/karl/git/BarleyGermination-KK/")
#2020
...
CH2_LD=rbind(CH2_LD_groupB,CH2_LD_groupA)
...
CH4_LD<-rbind(CH4_LD_groupA_real,CH4_LD_groupB_real)
...
LDheatmap.highlight(CH5_LD, i = 10, j = 32, col = "red",lwd=2,flipOutline=FALSE, crissCross=FALSE)
LDheatmap.highlight(CH5_LD, i = 33, j = 50, col = "red",lwd=2,flipOutline=FALSE, crissCross=FALSE)
LDheatmap.highlight(CH5_LD, i = 51, j = 124, col = "red",lwd=2,flipOutline=FALSE, crissCross=FALSE)
...
gmt<-WinterGM%>%filter(SNP%in%c(marker_table[marker_table$chrom=="5"&marker_table$trait=="PHS",]$marker))

gdt0<-WinterGD%>%select(gmt)%>%as.matrix()
...
CH5_LD=rbind(CH5_LD_groupA_real,CH5_LD_groupBQsd1_real,CH5_LD_groupC_real,CH5_LD_groupD_real)
```
...
LD_groups<-rbind(CH2_LD,CH4_LD,CH5_LD,CH7_LD)

collapse_table<-LD_groups%>%group_by(Ch,group)%>%arrange(Position)%>%reframe(SNP_marker=paste0(SNP[c(1)],", ",SNP[c(n()/2)],", ",SNP[c(n())]),bp_range=paste0(min(Position),"-",max(Position)),ld)
  collapse_table%>%unique() %>%ungroup()               
LD_groups%>%ungroup()
LD_groups[,1:3]
...
test<-Genetic_map%>%right_join(m3%>%select(marker,chrom,pos),by=c("chrom","marker"))%>%tidyr::drop_na(cM)%>%unique()
marker_table2$SNP
View(marker_table2)
m3

marker_table2<-m3%>%filter(p.value<FDR_rate)%>%left_join(test%>%select(!pos),by=(c("chrom","marker")))%>%arrange(chrom,pos)%>%mutate(marker_collapse=plyr::mapvalues(marker,from=conversion$SNP,to=conversion$SNP_marker))%>%dplyr::select(!marker)%>%unique()%>%reframe(.by = c(chrom,marker_collapse),pos_range=paste0(min(pos),"-",max(pos)),cm_range=paste0(min(cM),"-",max(cM)),PM_dates = toString(unique(PM)),trait=toString(unique(trait)),year=toString(unique(year)),testmin=round(min(maf),3),testmax=round(max(maf),3),PVEmin=round(min(expl.var),3),PVEmax=round(max(expl.var),3),Pvalue=max(round(p.value,4)))%>%
...
#QTL_Disease<-sig_hits_markers%>%mutate(SNP.2=SNP,Breed="1")%>%dplyr::select(SNP,SNP.2,trait,Chr,Pos,Breed,signif)%>%rename(Associated.marker=SNP,SNP.reference=SNP.2,Trait=trait,CHR=Chr,BP=Pos,Reference=signif)%>%
  mutate(CHR=as.character(CHR),BP=as.integer(BP))
...
tt<-cbind(output_qtls,tmp.split)%>%as.data.frame()
...
names(tt)[14]<-"significance_scores"
names(tt)[15]<-"gene_description1"
names(tt)[16]<-"test2"
names(tt)[17]<-"test3"
names(tt)[18]<-"test4"
names(tt)[19]<-"test5"
...
#View(tt)
descr2<-tt%>%dplyr::filter(grepl('_Description2', test2),grepl('_Gene_id', test3))%>%rename(gene_description2=16,gene_ID=17,gene_ID_2=18,GO_term=19,Confidence_primary=20,Confidence_secondary=21)%>%dplyr::select(!c("9","10"))
...
output_qtls<-plyr::rbind.fill(tt_descr1,descr)


# str_split_fixed(output_qtls[,c(7:13,15:18)],"\\=",n=2)[,2]
# sapply(strsplit(as.character(h$tes), "\\."), "[[", 2)
# sapply(strsplit(output_qtls[,c(7:13,15:18)], "\\="), "[[", 1)


  output_qtls[,"significance_scores"]<-stringr::str_split_fixed(output_qtls[,"significance_scores"], "=",n=2)[,2]

output_qtls[,"gene_description1"]<-stringr::str_split_fixed(output_qtls[,"gene_description1"], "=",n=2)[,2]
output_qtls[,"gene_ID"]<-stringr::str_split_fixed(output_qtls[,"gene_ID"], "=",n=2)[,2]
output_qtls[,"gene_ID_2"]<-stringr::str_split_fixed(output_qtls[,"gene_ID_2"], "=",n=2)[,2]

output_qtls[,"GO_term"]<-stringr::str_split_fixed(output_qtls[,"GO_term"], "=",n=2)[,2]
output_qtls[,"Confidence_primary"]<-stringr::str_split_fixed(output_qtls[,"Confidence_primary"], "=",n=2)[,2]
output_qtls[,"Confidence_secondary"]<-stringr::str_split_fixed(output_qtls[,"Confidence_secondary"], "=",n=2)[,2]
 
output_qtls[,"gene_description2"]<-stringr::str_split_fixed(output_qtls[,"gene_description2"], "=",n=2)[,2]
output_qtls[,"gene_description3"]<-stringr::str_split_fixed(output_qtls[,"gene_description3"], "=",n=2)[,2]
output_qtls[,"gene_description4"]<-stringr::str_split_fixed(output_qtls[,"gene_description4"], "=",n=2)[,2]   
output_qtls[,"gene_description5"]<-stringr::str_split_fixed(output_qtls[,"gene_description5"], "=",n=2)[,2]  
...
names(mrna_output)
#View(object)
object<-plyr::rbind.fill(mrna_output,hc_output,cds_output)%>%tidyr::drop_na(SNP.reference)
library(stringr)
object
tt<-object%>%group_by(trait,SNP.reference,gene_ID)%>%dplyr::select(grep("gene_description", names(object), value=TRUE))%>%tidyr::drop_na(gene_description1)%>%ungroup() %>% mutate_if(is.character, str_trim)%>%unique() %>% mutate_if(is.character, str_trim)
...
table(ss$CHR)
ww
ee<-ww%>%full_join(ss,by=c("SNP.reference","gene_ID"),relationship ="many-to-many")%>%tidyr::drop_na(trait)%>%arrange(trait,CHR,BP)%>%group_by(SNP.reference,gene_description)%>%slice(1)%>%ungroup()%>%arrange(CHR,BP)
ee
...
marker_table2[marker_table2$SNP=="JHI_Hv50k_2016_64721",]$Gene_candidate<-c("Mitogen-activated protein kinase(HORVU.MOREX.r2.2HG0082040)")

marker_table2[marker_table2$SNP=="SCRI_RS_135633, JHI_Hv50k_2016_90999, JHI_Hv50k_2016_141424",]$Gene_candidate<-"Dimeric alpha-amylase inhibitor(HORVU.MOREX.r2.2HG0176960)"
...
marker_table2[marker_table2$Chr=="4",]
marker_table2[marker_table2$SNP=="JHI_Hv50k_2016_262688, JHI_Hv50k_2016_262688, JHI_Hv50k_2016_262937",]$Gene_candidate<-"QGpc.StMo-4H(Protein Content)"
...
#CH5
marker_table2[marker_table2$Chr=="5",]
...
marker_table2[marker_table2$SNP=="Qsd1, JHI_Hv50k_2016_308790, JHI_Hv50k_2016_308859",]$Gene_candidate<-"HvAlaAT, abscisic aldehyde oxidase 3(HORVU.MOREX.r2.5HG0399320)"

marker_table2[marker_table2$SNP=="JHI_Hv50k_2016_310278, JHI_Hv50k_2016_312231, JHI_Hv50k_2016_314355",]$Gene_candidate<-c("1,4-alpha-glucan branching enzyme GlgB(HORVU.MOREX.r2.5HG0404420), GRAM-containing/ABA-responsive protein(HORVU.MOREX.r2.5HG0404090)")


marker_table2[marker_table2$SNP%in%c("JHI_Hv50k_2016_507218, JHI_Hv50k_2016_507370, JHI_Hv50k_2016_508103"),]$Gene_candidate<-"beta-amylase(HORVU.MOREX.r2.7HG0614220)"
marker_table2[marker_table2$Chr=="7",]$SNP
marker_table2[marker_table2$SNP%in%c("JHI_Hv50k_2016_450348"),]$Gene_candidate<-"ABA responsive binding factor(HORVU.MOREX.r2.7HG0533970)/ Glucan endo-1,3-beta-glucosidase,putative(HORVU.MOREX.r2.7HG0535140)/ Seed maturation protein(HORVU.MOREX.r2.7HG0535020)"

marker_table2[marker_table2$SNP%in%c("JHI_Hv50k_2016_486280"),]$Gene_candidate<-"Endoglucanase(HORVU.MOREX.r2.7HG0584740)"

marker_table2[marker_table2$SNP%in%c("JHI_Hv50k_2016_499737"),]$Gene_candidate<-"Far-red impaired responsive (FAR1) family protein(HORVU.MOREX.r2.7HG0609560)"

marker_table2[marker_table2$SNP%in%c("JHI_Hv50k_2016_499926"),]$Gene_candidate<-"beta glucosidase 17(HORVU.MOREX.r2.7HG0609800)"
marker_table2[marker_table2$SNP%in%c("JHI_Hv50k_2016_499926"),]$Gene_candidate
