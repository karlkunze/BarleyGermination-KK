...
GI21_BLUPS<-BLUP_vals(GI_2021_GWAS)
...
STP_all_BLUP<-rbind(GE20_BLUPS,GI20_BLUPS,GE21_BLUPS,GI21_BLUPS,GI_C_BLUPS,GE_C_BLUPS,PHS_BLUP)%>%mutate(notes="BLUP values")
...
comb[comb$PM==3,]$PM<-5
test<-comb%>%mutate(PM_date=paste("PM",as.character(PM)))%>%dplyr::select(!c(status,ID,std.error))%>%pivot_wider(names_from ="trait",values_from = c("predicted.value"))%>%select(!PM)%>%pivot_wider(names_from = "PM_date",values_from = c("PHS","GE","GI"))
#View(test)
...
FDR_rate
t<-WinterPerTPGWAS2%>%filter(p.value<0.0003872807)%>%select(names(markers1))
...
LDheatmap.highlight(CH5_LD, i = 10, j = 29, col = "red",lwd=2,flipOutline=FALSE, crissCross=FALSE)
LDheatmap.highlight(CH5_LD, i = 29, j = 44, col = "red",lwd=2,flipOutline=FALSE, crissCross=FALSE)
LDheatmap.highlight(CH5_LD, i = 44, j = 118, col = "red",lwd=2,flipOutline=FALSE, crissCross=FALSE)
...
marker_table2[marker_table2$SNP=="JHI_Hv50k_2016_275586, JHI_Hv50k_2016_276615, JHI_Hv50k_2016_276209",]$Gene_candidate<-c("	
...
marker_table2[marker_table2$SNP=="JHI_Hv50k_2016_308652, JHI_Hv50k_2016_308798, JHI_Hv50k_2016_310278",]$Gene_candidate<-"HvAlaAT-Aminotransferase(HORVU.MOREX.r3.5HG0481320.1), abscisic aldehyde oxidase 3(HORVU.MOREX.r2.5HG0399320)"

marker_table2[marker_table2$SNP=="JHI_Hv50k_2016_310358, JHI_Hv50k_2016_312229, JHI_Hv50k_2016_314246",]$Gene_candidate<-"GRAM-containing/ABA-responsive protein(HORVU.MOREX.r3.5HG0487770.1), 1,4-alpha-glucan branching enzymeGlgB(HORVU.MOREX.r3.5HG0488090.1),root meristem growth factor(HORVU.MOREX.r3.5HG0486840.1)"
...
marker_table2[marker_table2$SNP%in%c("JHI_Hv50k_2016_450348, JHI_Hv50k_2016_450348, JHI_Hv50k_2016_486280"),]$Gene_candidate<-"beta-amylase(HORVU.MOREX.r2.7HG0614220)/ABA responsive binding factor(HORVU.MOREX.r2.7HG0533970)/ Glucan endo-1,3-beta-glucosidase,putative(HORVU.MOREX.r2.7HG0535140)/ Seed maturation protein(HORVU.MOREX.r2.7HG0535020)"
...
marker_table2[marker_table2$SNP%in%c("JHI_Hv50k_2016_507370"),]$Gene_candidate<-"beta glucosidase 17(HORVU.MOREX.r2.7HG0609800)"
