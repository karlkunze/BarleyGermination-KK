...
GE20<-convert(GE_2020_GWAS)%>%unique()%>%mutate(chrom=ifelse(is.na(chrom),8,chrom))
GE20[is.na(GE20$pos),]

table(GE20$ID)
GI20<-convert(GI_2020_GWAS)%>%unique()%>%mutate(chrom=ifelse(is.na(chrom),8,chrom))
table(GI20$ID)
GE21<-convert(GE_2021_GWAS)%>%unique()%>%mutate(chrom=ifelse(is.na(chrom),8,chrom))
table(GE21$ID)
GI21<-convert(GI_2021_GWAS)%>%unique()%>%mutate(chrom=ifelse(is.na(chrom),8,chrom))
table(GE21$ID)

GI_C<-convert(GI_STP_combined)%>%unique()%>%mutate(chrom=ifelse(is.na(chrom),8,chrom))


table(GI_C$ID)
GE_C<-convert(GE_STP_combined)%>%unique()%>%mutate(chrom=ifelse(is.na(chrom),8,chrom))
#accidently labeled GE with GI here, see if I can change this

GE_C$ID<-gsub("GI","GE",GE_C$ID)

PHS<-GWAS_phs$gwas.all%>%arrange(chrom,pos)%>%mutate(chrom=ifelse(is.na(chrom),8,chrom))%>%mutate(ordinal=row_number(.),fdr.pvalues=p.adjust(p.value,method="fdr"))

PHS$ID<-GWAS_phs$trait

STP_all<-rbind(GE20,GI20,GE21,GI21,GI_C,GE_C,PHS)%>%mutate(notes="none")%>%group_by(ID)%>%arrange(chrom,pos)%>%mutate(ordinal=1:n())%>%ungroup()
View(STP_all[STP_all$chrom==2,])
hist(STP_all$pos)
...
table(WinterPerTPGWAS2[1])
table(WinterPerTPGWAS2$ID)
WinterPerTPGWAS2%>%ungroup()%>%filter(!trait=="PHS")%>%mutate(trait=ifelse(trait=="GI0","GI",trait))%>% ggplot(aes(ordinal, log10PVal, color = as.factor(PM), shape = year))+geom_point()+ labs(title = NULL)+
 ...
 geom_vline(xintercept = ALaAT_ord, color = 'red',linetype='longdash')+
  ...
  ylab('-log(p-value)')+xlab('Chromosome')+ geom_hline(yintercept = -log10(FDR_rate),linetype="longdash")+geom_hline(yintercept = -log10(0.05/7666) ,alpha=0.2,color="red")+theme_bw()+
...
gdat0<-as(gdt0,"SnpMatrix")
gdat0
LDheatmap(gdat0,genetic.distances=gmt$Position,title="LD on CH 2",color=rgb.palette(18),SNP.name = c(gmt$SNP),add.map = TRUE,text=TRUE)
...
#title -- Linkage Disequilribium on Chr 5H
CH5_LD<-LDheatmap(gdat,genetic.distances=gmt$Position,color=grey.colors(20),title=NULL,SNP.name = c(H5_1,AlaAT_L214F,PHS_GE),add.map = FALSE,text=FALSE,flip = FALSE)
...
## 6H
```{r}
marker_table[marker_table$chrom=="6",]$marker
gdt<-WinterGD%>%dplyr::select(marker_table[marker_table$chrom=="6",]$marker)%>%as.matrix()
gmt<-WinterGM%>%filter(SNP%in%c(marker_table[marker_table$chrom=="6",]$marker))
...
dim(gdat)
max_marker$SNP
LDheatmap(gdat,genetic.distances=gmt$Position,color=rgb.palette(18),title="LD on CH 6",SNP.name = c(min_marker$SNP,max_marker$SNP),add.map = FALSE,text=FALSE)
...
gdat0<-as(gdt0,"SnpMatrix")
gdat0
LDheatmap(gdat0,genetic.distances=gmt$Position,title="LD on CH 6",color=rgb.palette(18),SNP.name = c(gmt$SNP),add.map = TRUE,text=TRUE)


CH6_LD_groupA=data.frame(SNP=gmt$SNP,Ch="6",group="A",Position=gmt$Position,ld=0.95)
...
LD_groups<-rbind(CH1_LD_groupA,CH2_LD,CH3_LD_groupA,CH4_LD,CH5_LD,CH6_LD_groupA,CH7_LD)
...
collapse_table
conversion<-LD_groups[,1:3]%>%ungroup()%>%left_join(collapse_table,by=c("Ch","group"))%>%unique()
##
conversion
markers
m3<-markers
...
QTL_Disease<-markers%>%mutate(Associated.marker=marker,SNP.reference=marker,Breed="1",CHR=chrom,BP=pos,Reference=ID)%>%select(Associated.marker,SNP.reference,trait,Breed,CHR,BP,Reference)%>%arrange(CHR,BP)%>%drop_na(BP)
names(QTL_Disease)
QTL_Disease$CHR
...
marker_table2[marker_table2$SNP%in%c("SCRI_RS_135619", "JHI_Hv50k_2016_78907", "JHI_Hv50k_2016_78947"),]$Gene_candidate<-"Anthocyanin 3-O-beta-glucosyltransferase	HORVU.MOREX.r3.2HG0113240.1"
