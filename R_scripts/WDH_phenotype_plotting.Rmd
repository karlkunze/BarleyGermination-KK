...
q1.T<-ggplot(data=df, aes(x=TP, y=value,group=taxa,linetype=Check,color=Family)) +
  ...
  theme(plot.title = element_text(hjust = 0.5),axis.title.x=element_blank(),axis.title.y=element_blank(),legend.key.height= unit(0.6, 'cm'),legend.key.width= unit(0.2, 'cm'))
legend=get_legend(q1.T)
return(legend)
}
legend<-Sub_legend('Flavia','GIscale',lw,lh,flt,flx)
plot(legend)

Sub<-function(c,trait,scale_value,x,y){
size.x=x
size.y=y
TP_all_melt=melt(TP_all_WDH20_BLUE_all, id.vars =c("taxa","TP","Pedigree","Check"), measure.vars = c(trait))
TP_all_melt$TP=sapply(str_split(TP_all_melt$TP,"P",2),`[`,2)
...
q<-ggplot(data=df, aes(x=TP, y=value,group=taxa,linetype=Check,color=Family)) +
...
#Scala
q1=Sub('Flavia','GIscale',ylim(-7.5,1),0,8)
q2=Sub('Scala','GIscale',ylim(-7.5,1),0,0)
q3=Sub('SY Tepee','GIscale',ylim(-7.5,1),8,8)
q4=Sub('Wintmalt','GIscale',ylim(-7.5,1),8,0)
...
anno<-annotate_figure(figureT,top = text_grob("GI Scale", size = 14), bottom = text_grob("Time Point", size = 12))

anno
plot(legend)
q1=Sub('Flavia','GE',ylim(-1,0.05))
q2=Sub('Scala','GE',ylim(-1,0.05))
q3=Sub('SY Tepee','GE',ylim(-1,0.05))
q4=Sub('Wintmalt','GE',ylim(-1,0.05))
q3
blankPlot <- ggplot()+geom_blank(aes(1,1)) + 
  cowplot::theme_nothing()
...
anno<-annotate_figure(figureT,top = text_grob("3 Day GE BLUEs", face = "bold", size = 14), bottom = text_grob("Time Point", size = 12))
