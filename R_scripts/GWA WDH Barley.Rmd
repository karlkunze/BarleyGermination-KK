...
```{r}
#scald
wdh_scald.lmer=lmer(Scald~1+(1|Pedigree:Entry),data=WDH20_phenoF)
summary(wdh_scald.lmer)
AIC(wdh_scald.lmer)
anova(wdh_scald.lmer)
#winter hardiness percentage
wdh_wh.lmer=lmer(wh_percent~(1|Entry)+Location+Check,data=WDH20_phenoF)
summary(wdh_wh.lmer)
AIC(wdh_wh.lmer)
anova(wdh_wh.lmer)
#heading data
...
anova(wdh_HD.lmer)
#height
wdh_Ht.lmer=lmer(Ht~(1|Entry)+Location+wh_percent,data=WDH20_phenoF)
summary(wdh_Ht.lmer)
AIC(wdh_Ht.lmer)
anova(wdh_Ht.lmer)
...
Cullis_H2_scald=function(model){
  library(arm)
  ses<- se.ranef(model)$`Pedigree:Entry` #where 'm' is your model object from 'lmer' (replace 'genotypes' with whatever you call your individuals in the data)
  v_BLUP<- ses^2
  sigma2_g=VarCorr(model, comp="Variance")$`Pedigree:Entry`[1]
  Reliability<- 1- v_BLUP/ (2*sigma2_g)  #where sigma2_g is the genetic variance estimated with the model saved in 'm'
  H2<- round(mean(Reliability),3) #This is equivalent to broad-sense heritability on the line-mean (or family-mean, if your individuals are non-inbred families) basis
  H2
}
...
Cullis_H2_scald(wdh_scald.lmer)#0.728
Cullis_H2(wdh_wh.lmer)#0.706
Cullis_H2(wdh_HD.lmer)#0.915
Cullis_H2(wdh_Ht.lmer)#0.695 <-there's a data issue here
...
colnames(WDH_BLUEs_2020) = c('Entry','PHS','GETP1','GETP2','GETP3','GETP4',
                             'GITP1','GITP2','GITP3','GITP4','GETP1_5D','GETP2_5D','GETP4_5D','GETP4_5D','GITP1_5D','GITP2_5D','GITP3_5D','GITP4_5D','scaldmeans','wh_means','HD_means','PM_means','Ht_means')
...
gapit_blup_scald=function(mod,trait){
  df=ranef(mod)$'Entry:Pedigree'+fixef(mod)[[1]]; 
  df=cbind(rownames(df), df); 
  colnames(df)=c("taxa", trait); 
  df[,1]=as.character(df[,1])
  df
...
wdh_scald.blups=gapit_blup_scald(wdh_scald.lmer, "Scald")
...
#GWAS.GETP1.blups.mlm  =GAPIT(Y= GETP1.blups, GD=myGD20, GM=myGM20, PCA.total=2,file.output=T, Geno.View.output=T, model="MLM", Major.allele.zero = F)

GWAS.GETP1.blups.mlmm  =GAPIT(Y= GETP1.blups, GD=myGD20, GM=myGM20, PCA.total=2,file.output=T, Geno.View.output=T, model="MLM", Major.allele.zero = F)
...
GWAS.GITP2.blues.mlmm  =GAPIT(Y= GITP2.blues, GD=myGD20, GM=myGM20, PCA.total=2,file.output=T, Geno.View.output=T, model="MLM", Major.allele.zero = F)
...
GWAS.HD.blups.mlmm  =GAPIT(Y= HD.blups, GD=myGD20, GM=myGM20, PCA.total=2,file.output=T, Geno.View.output=T, model="FarmCPU", Major.allele.zero = F)
