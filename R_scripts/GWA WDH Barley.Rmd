...
wdh_scald.lm=lm(Scald~Entry,data=WDH20_phenoF)
anova(wdh_scald.lm)

wdh_wh.lm=lm(wh_percent~Entry+Location+PM,data=WDH20_phenoF)
anova(wdh_wh.lm)
#cant include check because there isn't balance
wdh_HD.lm=lm(HD~Entry+Location,data=WDH20_phenoF)
anova(wdh_HD.lm)
wdh_Ht.lm=lm(Ht~Entry+Location+wh_percent,data=WDH20_phenoF)
anova(wdh_Ht.lm)
...
```{r}
#scald
wdh_scald.lmer=lmer(Scald~(1|Entry)+Location+Check,data=WDH20_phenoF)
...
anova(wdh_wh.lmer)
#heading data
wdh_HD.lmer=lmer(HD~(1|Entry)+Location+Check,data=WDH20_phenoF)
summary(wdh_HD.lmer)
AIC(wdh_HD.lmer)
anova(wdh_HD.lmer)
#height
...
GWAS.scald.blups.mlm =GAPIT(Y= , GD=myGD20, GM=myGM20, PCA.total=2,file.output=T, Geno.View.output=T, model="MLM", Major.allele.zero = F)
