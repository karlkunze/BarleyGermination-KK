...
yGAPIT <- GAPIT(G=myG, output.numerical=TRUE)
yGAPIT$G
#df change
wmb_gt_t<-wmb_gt
wmb_gt_t
...
wmbGAPIT<- GAPIT(G=rbind(wmb_gt[1,],wmb_gt), output.numerical=TRUE)

wmbGAPIT$GM<-wmb_gt_t%>%dplyr::select(`rs#`,chrom,pos)%>%rename(SNP=`rs#`,Chromosome=chrom,Position=pos)
dim(wmbGAPIT$GM)
dim(wmbGAPIT$GD)

colnames(wmbGAPIT$GD)=gsub("\\.","_",colnames(wmbGAPIT$GD))
...
wmbGAPIT$GD[1:5,1:5]
snpgdsCreateGeno("test3.gds", genmat = as.matrix(round(wmbGAPIT$GD,0)), sample.id=rownames(wmbGAPIT$GD), snp.id = colnames(wmbGAPIT$GD), snp.chromosome = names(wmbGAPIT$GM$Chromosome),
