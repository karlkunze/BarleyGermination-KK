...
#REFERENCE 2
# AlaT_markers_1<-AlaT_markers_1%>%as.data.frame%>%tibble::add_column("rs#"="Qsd1","alleles"="C/G","chrom"="5H","pos"=442160000,"strand"="+",.before =1)
#REFERENCE 3
AlaT_markers_1<-AlaT_markers_1%>%as.data.frame%>%tibble::add_column("rs#"="Qsd1","alleles"="C/G","chrom"="5H","pos"=435636564,"strand"="+",.before =1)

#View(AlaT_markers_1)
rownames(AlaT_markers_1)<-NULL
AlaT_markers_1<-AlaT_markers_1%>%dplyr::select(!"unknown-omit")
AlaT_markers_1
table(t(AlaT_markers_1))
```
#input the new data
```{r}
...
wmb_gt[wmb_gt$`rs#`=="Qsd1",]$alleles<-"C/G"
...
Morex2019[Morex2019$Marker=="Qsd1",]
...
#ref 3 is 435636564
myGM20[myGM20$SNP=="Qsd1",2:3]<-c(5,435636564)
...
wmbGAPIT<- GAPIT(G=rbind(wmb_gt[1,],wmb_gt),output.numerical = FALSE,file.output = FALSE)
...
wmbGAPIT$GM<-wmb_gt_t%>%dplyr::select(`rs#`,chrom,pos)%>%dplyr::rename(SNP=`rs#`,Chromosome=chrom,Position=pos)
...
wmb_num<-wmbGAPIT
wmb_num$GD[wmb_num$GD$Qsd1==1,]$Qsd1<-c(0,)

correction<-rownames(wmb_num$GD[wmb_num$GD$Qsd1==1,])
correction
AlaT_markers$GID
allele<-AlaT_markers%>%mutate(GID=toupper(GID))%>%filter(GID%in%correction)
allele_C<-allele%>%filter(Allele_bp=="C")
allele_G<-allele%>%filter(Allele_bp=="G")


wmb_num$GD[rownames(wmb_num$GD)%in%allele_C$GID,]$Qsd1<-0
wmb_num$GD[rownames(wmb_num$GD)%in%allele_G$GID,]$Qsd1<-2
