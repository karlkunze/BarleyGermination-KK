...
rownames(geno)
as.integer(geno$bp)
geno <- MatrixGenotypeReader(genotype = as.matrix(geno[,-c(1:3)]), snpID = as.integer(rownames(geno)),
                             chromosome = geno$chrom,position = geno$bp,
                             scanID = colnames(geno)[-c(1:3)])
genoData <- GenotypeData(geno)

library(SNPRelate)
WinterGD[1:3,1:10]
wmbGAPIT$A[1:5,1:5]
snpgdsCreateGeno("test6.gds", genmat = as.matrix(round(WinterGD[-1],0)), sample.id=rownames(WinterGD$taxa), snp.id = colnames(WinterGD[-1]), snp.chromosome = names(WinterGM$Chromosome),
                 snp.position = as.integer(WinterGM$Position), snpfirstdim =FALSE)
(gdsfile <- snpgdsOpen("test6.gds"))
...
HapMap_geno <- GdsGenotypeReader(filename = "test6.gds")
HapMap_geno 
...
illumina_snp_annot
anno<-geno%>%dplyr::select(marker,chrom,bp)%>%arrange(chrom,bp)
...
str(HapMap_ASW_MXL_KINGmat)
str(HapMap_ASW_MXL_KINGmat[1:5,1:5])

as.matrix(wmbGAPIT$A)
# run PC-AiR
str(HapMap_genoData)
which(colnames(wmbGAPIT$A)%in%WinterGD$taxa)

wmbGAPIT$A[-479,-479]
View(wmbGAPIT$A)
WinterGD$taxa
rownames(wmbGAPIT$A)[479]<-"LIGHTNING"
colnames(wmbGAPIT$A)[479]<-"LIGHTNING"
str(wmbGAPIT$A)
HapMap_genoData
HapMap_genoData
