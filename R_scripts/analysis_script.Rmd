...
table(pheno_1$PM_date)
names(pheno_1)
pheno=pheno_1%>%filter(trait=="GI",PM_date=="12",year=="2021")%>%dplyr::select(taxa,value)
pheno[grep("DH",pheno$taxa),]$taxa<-"LIGHTNING"
pheno$taxa<-toupper(pheno$taxa)
library(GWASpoly)
?GWASpoly.data()
names(geno)[1:4]

geno_m=as.matrix(geno[,-c(1:3)])
geno
map=geno[,c("marker","chrom","bp")]
pheno_1%>%filter()
...
GWASpoly.data(map=map,pheno,geno=geno_m,ploidy = 2)
?GWASpoly.data()
str(data)
str(data@map)
```
