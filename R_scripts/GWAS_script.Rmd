...
AllDHBluesPerYear
 filter(type == 'BLUE') %>% mutate(year = factor(year, levels = c('2020','2021','2020/2021')))%>%
  plyr::join(WinterGD[,c('taxa','Qsd1')]) %>% filter(Qsd1!= 1) %>% mutate(Qsd1= ifelse(Qsd1==2,'Dormant','Nondormant')) %>%
...
#marker effects
marker_ef<-dim%>%as.data.frame()%>%dplyr::select("Qsd1","JHI_Hv50k_2016_308762")%>%tibble::rownames_to_column()%>%rename(GID=rowname)%>%mutate(Qsd1=as.factor(Qsd1),JHI_Hv50k_2016_308762=as.factor(JHI_Hv50k_2016_308762))
marker_ef
...
gwasSTP<- gwas.asreml(pheno.data = pre.aprAA$pheno.data,resp = "GI",
 gen = "GID", Kinv = pre.aprAA$Kinv,
Q =pre.aprAA$Q, npc = 2, geno.data = pre.aprAA$geno.data,
map.data = pre.aprAA$map.data, pvalue.thr = 9.689937e-07,
...
gwasSTP<- gwas.asreml(pheno.data = pre.aprAA$pheno.data,resp = "GI",
 gen = "GID", Kinv = pre.aprAA$Kinv,
Q =pre.aprAA$Q, npc = 2, geno.data = pre.aprAA$geno.data,
map.data = pre.aprAA$map.data, pvalue.thr = 5.734206e-05,
bonferroni = FALSE,fixedf = fixed,residual=resid,randomf = rand,workspace='2Gb')
##
...
hist(simple$GI)
simple$ND<-as.character(simple$Check)
simple$GI_l<-simple$GI^0.5
hist(simple$GI_l)
pre.aprAA <- pre.gwas(pheno.data=simple, indiv = "GID",
resp = "GI_l", geno.data = gd,map.data = genoMp,
maf = maf_lim, marker.callrate = 0.2, ind.callrate = 0.2,
method = "VanRaden", rename.markers = TRUE, impute = FALSE, Q.method = "K",heterozygosity = 0.20)
##
...
random=NULL
fixed=c("Location:Row","Location:Column","Location")
resid=NULL
gwasSTP<- gwas.asreml(pheno.data = pre.aprAA$pheno.data,resp = "GE",
 gen = "GID", Kinv = pre.aprAA$Kinv,
Q = pre.aprAA$Q, npc = 2, geno.data = pre.aprAA$geno.data,
...
simple<-simple%>%dplyr::select(SourcePLOT,GID,Year,PM_date,replication,Location,Check,Day1Germ,scald,Qsd1,GE,GI,GI_0,JHI_Hv50k_2016_308762,Row,Column,spot_bloch,ws_percent,AssayNumber)%>%mutate(GE_tr=abs(GE - mean(GE,na.rm = TRUE)))%>%mutate(groups=as.factor(cut(simple$AssayNumber,5)))
hist(simple$GE_bi)
...
gwasSTP<- gwas.asreml(pheno.data = pre.aprAA$pheno.data,resp = "GE",
 gen = "GID", Kinv = pre.aprAA$Kinv,
Q =  NULL, npc = 7, geno.data = pre.aprAA$geno.data,
map.data = pre.aprAA$map.data, bonferroni = TRUE,
fixedf = fixed,residual=resid,randomf = rand,workspace='2Gb')
...
simple<-simple%>%dplyr::select(SourcePLOT,GID,Year,PM_date,replication,Location,Check,Day1Germ,scald,Qsd1,GE,GI,GI_0,JHI_Hv50k_2016_308762,Row,Column,spot_bloch,ws_percent)%>%mutate(GE_tr=abs(GE - mean(GE,na.rm = TRUE)^-5))
...
Q =  pre.aprAA$Q, npc = 7, geno.data = pre.aprAA$geno.data,
map.data = pre.aprAA$map.data, bonferroni = TRUE,
fixedf = fixed,residual=resid,randomf = rand,workspace='2Gb',maxiter.update = 10)
...
GE2021_PM96
GE2021_PM96<-gwasSTP
GE2021_PM96$trait<-paste0(tr,"-",ty,"-PM_",tpm,": did_not_coverge")
GE2021_PM96$trait
table(GE_2021$PM_date)
```
### GE 2021 152
...
GI_2021=list(GI2021_PM5,GI2021_PM19,GI2021_PM33,GI2021_PM47,GI2021_PM68,GI2021_PM96,GI2021_PM152)
save(GI_2021,file="data/Genotype_data/ASRgwas_results/Single_TP_GI_2021_results.Rdata")

GE2021=list(GE2021_PM5,GE2021_PM12,GE2021_PM33,GE2021_PM68,GE2021_PM96,GE2021_PM152)
save(GE_2021,file="data/Genotype_data/ASRgwas_results/Single_TP_GE_2021_results.Rdata")
```
# COMBINED
...
GE_all$Year
##GI PM 5
simple<-GE_all%>%filter(PM_date=="12")
simple
gd=dim;genoMp=genoM;maf_lim=0.05;p.val=5e-05;bonfer=FALSE
#View(simple)
...
GE_12<-gwasSTP
GE_12$trait<-paste0(paste0("GE-",ty[1],"/",ty[2]),"-","PM_",tpm)
GE_12$trait
table(GE_all$PM_date)
```
### GE 47
...
resid="Year"
table(pre.aprAA$pheno.data$Year)
gwasSTP<- gwas.asreml(pheno.data = pre.aprAA$pheno.data,resp = "GE_tr",
 gen = "GID", Kinv = pre.aprAA$Kinv,
...
#GE COMBINED
```{r}
GE_STP_combined=list(GE_5,GE_12,GE_47,GE_96,GE_152)
