...
#packages
```{r}
library(rrBLUP);library(sommer);library(arm);library(lme4);library(Hmisc);library(plyr);library(readxl);
library(tibble);library(patchwork);library(ggplot2);library(fda) ; library(magic); 
library(drc);library(tidyr);library(ggh4x);library(dplyr)
...
# E  ungroup() %>% group_by(year, TP, trait) %>% group_modify(GWA_MLMM_fortidyR)
...
fg<- DH2020_1 %>% rbind(DH2021_1, DHcomb_1) %>%  filter(type == 'BLUE') %>%
   ungroup()%>%filter(year=="2020",TP=="TP2",trait=="GI")%>%rename(id=taxa)
DT<-fg
# unadjusted
#genotype
GT_1<-geno%>%as.data.frame()%>%select(any_of(DT$id))%>%t();A <- A.mat(GT_1) # additive relationship matrix
#back to phenotypes
DT<-DT%>%filter(id%in%rownames(A))%>%as.data.frame()

dim(A);dim(GT_1);dim(DT)

mix1 <- sommer::GWAS(value~1,random=~vsr(id,Gu=A),rcov=~units,data=DT, nIters=10,M=GT_1, gTerm = "u:id",
verbose = TRUE,n.PC = 2,getPEV=TRUE)
mix1
pmix1lot(mix1 $scores[,1])
names(which(mix1 $scores[,1]>5.5))

View(mix1$VarU[[1]])
mix1$sigmaSE
hist(as.data.frame(str(mix1$VarU)))

v<-mix1$VarU[[1]]
dim(v)
table(mix1$fitted)
names(WinterPerTPGWAS)
str(mix1)
mix1$scores
summary(mix1)
names(WinterPerTPGWAS)
...
MP=geno%>%select(marker,Chrom,bp)%>%rename(Locus=marker,Position=bp)
ms
MP2 <- merge(MP,ms,by="Locus",all.x = TRUE);
MP2
MP2=MP2%>%arrange(Chrom,Position)%>%drop_na(Position)
manhattan(MP2, PVCN = "value",pch=20,cex=.5,ylim=c(0,20))
...
head(DH2020_2021)
DH2020_2021$PM_date
GI_2021_5<-DH2020_2021%>%filter(Year=="2021",PM_date=="33")
simple<-GI_2021_5%>%dplyr::select(SourcePLOT,GI,GE,taxa,Year,PM_date,replication,Location)
simple<-simple%>%mutate(replication=as.factor(replication),Location=as.factor(Location),GID=toupper(taxa))
...
pre.apr <- pre.gwas(pheno.data = simple, indiv = "GID",
resp = "GI", geno.data = dim, map.data = genoM,
maf = 0.05, marker.callrate = 0.2, ind.callrate = 0.2,
method = "VanRaden", rename.markers = TRUE, impute = TRUE, Q.method = "K")

gwasS <- gwas.asreml(pheno.data = pre.apr$pheno.data,fixedf = "replication",
resp = "GI", gen = "GID", Kinv = pre.apr$Kinv,
Q = pre.apr$Q, npc = 4, geno.data = pre.apr$geno.data,
map.data = pre.apr$map, pvalue.thr = 5e-04,
bonferroni = FALSE)
library(asreml)
library(ASRdata)
library(ASRdata)
