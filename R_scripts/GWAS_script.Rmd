...
map.data = pre.aprAA$map.data, pvalue.thr = p.val,fixedf = fixed,residual=resid,randomf = random,workspace='2Gb')
...
gwasFIN <- select.marker(gwas.object = gwasSTP, geno.data.sel = Msel[,c(1,2,15)],
...
Q = NULL, npc = 5, geno.data = pre.aprAA$geno.data,
map.data = pre.aprAA$map.data, pvalue.thr = p.val,
bonferroni = 5e-05,fixedf = fixed,residual=resid,randomf = rand,workspace='2Gb')
...
qq.plot(gwas.table = gwasSTP$gwas.all)
manhattan.plot(gwas.table = gwasSTP$gwas.all, pvalue.thr = 5e-5,
point.size = 1.2)
sel.markers
