...
#marker effects
df_begin<-DH2020_2021%>%mutate(GID=toupper(taxa))%>%filter(!Location=="Spring")
marker_ef<-dim%>%as.data.frame()%>%select("Qsd1")%>%tibble::rownames_to_column()%>%rename(GID=rowname)%>%mutate(Qsd1=as.factor(Qsd1))
marker_ef
table(df_begin$PM_date,df_begin$Year)

names(DH2020_2021)

simple<-df_begin%>%mutate(GID=toupper(taxa))%>%left_join(marker_ef,by="GID")#%>%dplyr::filter(Year=="2021",PM_date=="47")

simple<-simple%>%dplyr::select(SourcePLOT,GI,GE,GID,Year,PM_date,replication,Location,Check,Qsd1,Day1Germ,scald)

simple<-simple%>%mutate(replication=as.factor(replication),Location=as.factor(Location),Check=as.factor(Check))
simple<-simple%>%mutate(GE_binomial=(ifelse(GE>0.95,1,0)))
simple$GE_binomial
pre.aprA <- pre.gwas(pheno.data = simple, indiv = "GID",
resp = "GE_binomial", geno.data = dim,map.data = genoM,
...
###
pr
pre.aprA$pheno.data$GE_binomial
pre.aprA$pheno.data$GE_binomial
gwasMTP <- gwas.asreml(pheno.data = pre.aprA$pheno.data,resp = "GE_binomial",family = "binomial",randomf = "PM_date",
 gen = "GID", Kinv = pre.aprA$Kinv,
Q = pre.aprA$Q, npc = 5, geno.data = pre.aprA$geno.data,
map.data = pre.aprA$map.data, pvalue.thr = 5e-04,
bonferroni = FALSE, fixedf =c("Year",("Year:PM_date"),"replication"))
library(ASRgwas)
###

# gwasS <- gwas.asreml(pheno.data = pre.apr$pheno.data,
# resp = "Day1Germ", gen = "GID", Kinv = pre.apr$Kinv,
