...
pre.aprAA <- pre.gwas(pheno.data=simple%>%filter(!ND=="2")%>%droplevels(), indiv = "GID",
resp = "Day1Germ", geno.data = gd,map.data = genoMp,
maf = maf_lim, marker.callrate = 0.2, ind.callrate = 0.2,
method = "VanRaden", rename.markers = TRUE, impute = FALSE, Q.method = "K",heterozygosity = 0.20)
##
...
fixed="Location"
resid=NULL
gwasSTP<- gwas.asreml(pheno.data = pre.aprAA$pheno.data,resp = "Day1Germ",
 gen = "GID", Kinv = pre.aprAA$Kinv,
Q = NULL, npc = 5, geno.data = pre.aprAA$geno.data,
map.data = pre.aprAA$map.data, pvalue.thr = p.val,
...
simple<-simple%>%dplyr::select(SourcePLOT,GID,Year,PM_date,replication,Location,Check,Day1Germ,scald,Qsd1,GE,GI,GI_0,JHI_Hv50k_2016_308762,Row,Column,spot_bloch,ws_percent)%>%mutate(GE_bi=ifelse(GE>0.6,1,ifelse(GE<0.4,0,NA)))
hist(simple$GE_bi)
...
pre.aprAA <- pre.gwas(pheno.data=simple%>%filter(!ND=="2")%>%droplevels(), indiv = "GID",
resp = "GE_bi", geno.data = gd,map.data = genoMp,
maf = maf_lim, marker.callrate = 0.2, ind.callrate = 0.2,
method = "VanRaden", rename.markers = TRUE, impute = FALSE, Q.method = "K",heterozygosity = 0.20)
##
...
gwasSTP<- gwas.asreml(pheno.data = pre.aprAA$pheno.data,resp = "GE_bi",
 gen = "GID", Kinv = pre.aprAA$Kinv,family = "binomial",
Q =  NULL, npc = 7, geno.data = pre.aprAA$geno.data,
map.data = pre.aprAA$map.data, pvalue.thr = p.val,
