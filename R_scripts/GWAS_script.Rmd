...
# Sum should = 9269, its now 7706 due to a higher sliding window bp of 25,000
dim(WinterGD)
...
library("ASRgenomics")
print("This isn'rt the real one")
data(geno.pine655)
...
#marker effects
marker_ef<-genoD%>%as.data.frame()%>%dplyr::select("Qsd1")%>%tibble::rownames_to_column()%>%rename(GID=rowname)%>%mutate(Qsd1=as.factor(Qsd1))
marker_ef
...
library(dplyr)
#we need to include
phs<-DH2020_2021%>%mutate(GID=toupper(taxa))%>%filter(!Location=="Spring")%>%left_join(marker_ef,by="GID")%>%mutate(type="base_data_single_step")%>%dplyr::select(GID,Location,SourcePLOT,Row,Check,Family,Year,phs)%>%left_join(marker_ef,by="GID")

simple<-phs%>%unique()
simple$Check<-as.factor(simple$Check)
...
pre.aprAA <- pre.gwas(pheno.data=simple, indiv = "GID",fixed="Check",
...
pre<-gwasSTP.phs$mod
pre$call$data<-pre.aprAA$pheno.data
BLUPS<-predict.asreml(pre,classify ="vm(GID, pre.aprAA$Kinv$GID)")
BLUPS
GWAS_phs$BLUPS<-BLUPS
...
pre<-gwasSTP$mod
pre$call$data<-pre.aprAA$pheno.data
BLUPS<-predict.asreml(pre,classify ="vm(GID, pre.aprAA$Kinv$GID)",aliased = TRUE)
...
pre.aprAA <- pre.gwas(pheno.data=simple, indiv = "GID",
resp = "GI", geno.data = gd,map.data = genoMp,
maf = maf_lim, marker.callrate = 0.2, ind.callrate = 0.1,
method = "VanRaden", rename.markers = TRUE, impute = FALSE, Q.method = "K",heterozygosity = 0.05)
##
table(gd)
...
print(paste0("data input: ",df$type%>%unique()))
simple
simple<-simple%>%dplyr::select(SourcePLOT,GID,Year,PM_date,replication,Location,Check,Day1Germ,scald,Qsd1,GE,GI,GI_0,Row,Column,spot_bloch,ws_percent)
...
print(paste0("data input: ",GI_2021$type%>%unique()))
#View(simple)
simple<-simple%>%dplyr::select(SourcePLOT,GID,Year,PM_date,replication,Location,Check,Day1Germ,scald,Qsd1,GE,GI,GI_0,Row,Column,spot_bloch,Maturity_date)%>%mutate(GI_lam=GI^1.2)
#View(simple)
hist(simple$GI_lam)
simple$ND<-as.character(simple$Check)
length(simple$GI_0)
...
simple<-simple%>%dplyr::select(SourcePLOT,GID,Year,PM_date,replication,Location,Check,Day1Germ,scald,Qsd1,GE,GI,GI_0,Row,Column,spot_bloch,ws_percent)%>%mutate(GE_tr=abs(GE - mean(GE,na.rm = TRUE)))
...
simple<-simple%>%dplyr::select(SourcePLOT,GID,Year,PM_date,replication,Location,Check,Day1Germ,scald,Qsd1,GE,GI,GI_0,Row,Column,spot_bloch,ws_percent)%>%mutate(GE)
#hist(simple$GE)
...
resp = "GE", geno.data = gd,map.data = genoMp,
maf = maf_lim, marker.callrate = 0.2, ind.callrate = 0.2,
method = "VanRaden", rename.markers = TRUE, impute = FALSE, Q.method = "K",heterozygosity = 0.20,diagonal.thr.large = 2,diagonal.thr.small=0)
##
hist(pre.aprAA $pheno.data$GE)
rand=NULL
fixed=c("Location:Row")
...
gwasSTP$heritability
pre<-gwasSTP$mod
pre.aprAA$pheno.data$GE
pre$call$data<-pre.aprAA$pheno.data%>%unique()
...
simple<-simple%>%dplyr::select(SourcePLOT,GID,Year,PM_date,replication,Location,Check,Day1Germ,scald,Qsd1,GE,GI,GI_0,Row,Column,spot_bloch,ws_percent,AssayNumber)%>%mutate(GE_tr=abs(GE - mean(GE,na.rm = TRUE)^-5))%>%mutate(groups=as.factor(cut(simple$AssayNumber,8)))
...
resp = "GE", geno.data = gd,map.data = genoMp,
maf = maf_lim, marker.callrate = 0.2, ind.callrate = 0.2,
method = "VanRaden", rename.markers = TRUE, impute = FALSE, Q.method = "K",heterozygosity = 0.20,diagonal.thr.large = 2,diagonal.thr.min=0)
##
...
GI2021_PM47
GI_2021_GWAS=list(GI2021_PM5,GI2021_PM12,GI2021_PM19,GI2021_PM33,GI2021_PM47,GI2021_PM68,GI2021_PM96,GI2021_PM152)

save(GI_2021_GWAS,file="data/Genotype_data/ASRgwas_results/Single_TP_GI_2021_results.Rdata")

GE_2021_GWAS=list(GE2021_PM5,GE2021_PM12,GE2021_PM19,GE2021_PM33,GE2021_PM47,GE2021_PM68,GE2021_PM96,GE2021_PM152)
...
gwasSTP<- gwas.asreml(pheno.data = pre.aprAA$pheno.data,resp = "GE",
 gen = "GID", Kinv = pre.aprAA$Kinv,
Q = pre.aprAA$Q, npc = 2, geno.data = pre.aprAA$geno.data,bonferroni = FALSE,
map.data = pre.aprAA$map.data, pvalue.thr =5e-05,
fixedf = fixed,residual=resid,randomf = random,workspace='2Gb')
...
GE_47<-gwasSTP
GE_47$trait<-paste0(paste0("GE-",ty[1],"/",ty[2]),"-","PM_",tpm)
GE_47$trait
table(GE_all$PM_date)
```
### GE 96
...
simple<-simple%>%dplyr::select(SourcePLOT,GID,Year,PM_date,replication,Location,Check,Day1Germ,scald,Qsd1,GE,GI,GI_0,Row,Column,spot_bloch,ws_percent)%>%mutate(GE_tr=abs(GE - mean(GE,na.rm = TRUE)),GE_lam=GE^8)
...
GE_96<-gwasSTP
GE_96$trait<-paste0(paste0("GE-",ty[1],"/",ty[2]),"-","PM_",tpm)
GE_96$trait
table(GE_all$PM_date)
```
...
GE_152<-gwasSTP
GE_152$trait<-paste0(paste0("GE-",ty[1],"/",ty[2]),"-","PM_",tpm)
GE_152$trait
table(GE_all$PM_date)
```
#GE COMBINED
